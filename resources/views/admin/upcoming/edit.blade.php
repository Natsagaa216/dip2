            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3>Edit Show
                            <a href="{{ url('admin/upcoming') }}" class="btn btn-primary btn-sm text-white float-end">BACK</a>
                        </h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ url('admin/upcoming/' . $product->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control" value="{{ old('name', $upcoming->show_name) }}" required>
                                    @error('name') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label for="name">Show-Date</label>
                                    <input type="text" name="name" class="form-control" value="{{ old('name', $upcoming->show_date) }}" required>
                                    @error('name') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label for="name">Show-Time</label>
                                    <input type="text" name="name" class="form-control" value="{{ old('name', $upcoming->show_time) }}" required>
                                    @error('name') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label for="name">Venue</label>
                                    <input type="text" name="name" class="form-control" value="{{ old('name', $upcoming->venue) }}" required>
                                    @error('name') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>
